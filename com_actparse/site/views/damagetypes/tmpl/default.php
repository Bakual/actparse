...
?>
<div class="com-actparse-damagetypes<?php echo $this->pageclass_sfx; ?> category-list">
	...
	<form action="<?php echo htmlspecialchars(Uri::getInstance()->toString()); ?>" method="post" id="adminForm"
		  name="adminForm" class="com-actparse-damagetypes__damagetypes">
		...
		<?php if (!count($this->items)) : ?>
			<div class="alert alert-info">
				...
				<?php echo Text::sprintf('COM_ACTPARSE_NO_ENTRIES', Text::_('COM_ACTPARSE_DAMAGETYPES')); ?>
			</div>
		<?php else : ?>
			<table class="com-actparse-damagetypes__table category table table-striped table-bordered table-hover">
				<thead>
				<tr>
				...
				<?php foreach ($this->items as $i => $item) : ?>
					<tr class="cat-list-row<?php echo $i % 2; ?>">
						<td>
							<a href="<?php echo Route::_('index.php?view=attacktypes&encid=' . $item->encid . '&' . $item->grouping . '&type=' . $item->type); ?>">
								<?php echo $item->type; ?>
							</a>
						</td>
						<?php foreach ($this->cols as $col) : ?>
		...
		<?php endif; ?>
		<?php echo LayoutHelper::render('blocks.pagination', array('view' => 'damagetypes', 'pagination' => $this->pagination, 'params' => $this->params)); ?>
